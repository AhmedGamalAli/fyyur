...
class Venue(db.Model):
    ...
    generes = db.Column(db.ARRAY(db.String()))
...
def venues():
  ...
  areas = [
    ven.get_city_and_state for ven in Venue.query.distinct(Venue.city, Venue.state).all()
  ]
  for a in areas:
    a["venues"] = [
      ven.self_to_dict for ven in Venue.query.filter_by(city=area["city"], state=area["state"]).all()
    ]
...
def search_venues():
  ...
  vens = Venue.query.filter(Venue.name.ilike("%" + search_term + "%")).all()
  response = {
    "count": len(vens),
    "data": [
        ven.search_result for ven in vens
    ]
  }
...
def show_venue(venue_id):
  ...
  return render_template('pages/show_venue.html', venue=v.venue_shows)
